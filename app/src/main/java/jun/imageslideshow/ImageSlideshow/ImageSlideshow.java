...
import jun.imageslideshow.test.Activity_1;
...
public class ImageSlideshow extends FrameLayout {
     ...
     * @param imageTitleBeanList
     */
    private void setViewList(final List<ImageTitleBean> imageTitleBeanList) {
        viewList = new ArrayList<>();
        for (int i = 0; i < count + 2; i++) {
            View view = LayoutInflater.from(context).inflate(R.layout.is_image_title_layout, null);
            ImageView ivImage = (ImageView) view.findViewById(R.id.iv_image);
            TextView tvTitle = (TextView) view.findViewById(R.id.tv_title);
            Button bt = (Button) view.findViewById(R.id.bt_click);
            bt.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    context.startActivity(new Intent(context, Activity_1.class));
                }
            });
            final int finalI = i;
            tvTitle.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    Toast.makeText(context, "title = " + imageTitleBeanList.get(finalI - 1).getTitle(), Toast.LENGTH_SHORT).show();
                    context.startActivity(new Intent(context, Activity_1.class));
                }
            });
